...
- **A part's photo must actually be a close-up of that part**, not another full-appliance shot. Check the image before using it, don't trust the filename — `raw/kitchen-tools/ACQUISITION-LOG.md`/`SOURCES.md` entries can mislabel a whole-product photo as a "control panel" image (seen: an AEG oven's "controls-representative.jpg" turned out to be a second full oven photo of a different model, which on the card reads as a second oven). If no genuine close-up exists for a part, drop that part rather than fill it with a near-duplicate appliance photo — an icon (or omitting the part) beats a misleading photo.
...
- [ ] Every part photo was actually opened and shows a close-up of that part, not another full-appliance shot.
